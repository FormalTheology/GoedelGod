...
\,\hfill Henkin semantics for both HOML and HOL \\[2em]
...
\vfill
Modal logic axioms \hfill Semantical condition 
...
\begin{frame}{Possibilist Quantification --- Varying Domains for Individuals}
...
\wedge \ldots \wedge  \textbf{exInW} {t^n} w \supset \textbf{exInW}{(f\,t^1\ldots t^n)} w)$}\\[1em]

\item cumulative domains \hfill \textcolor{Blue}{$\forall{x}\forall{v}\forall{w}  (\textbf{exInW} x v \wedge r v w \supset \textbf{exInW} x w)$}
\end{itemize}
\end{frame}
...
\item[\textcolor{Green}{Theorem 1}] Positive Properties are possibly
  ...
  \phi(x)]$ \\
\,\hfill by \textcolor{Blue}{A1($\supset$), A2}\\[2em]
...
\,\hfill by \textcolor{Blue}{A5, Lemma1, D3, T1} \\[2em]
\end{itemize}
\end{minipage}
\end{flushright}
\end{frame}
...
\end{block}
\vfill
\begin{block}{Question: \hfill Why is inconsistency not reported in philosophy papers?}
\end{block}
\end{frame}
