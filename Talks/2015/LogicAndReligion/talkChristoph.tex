...
\item \textcolor{red}{Ours:} Can computers (theorem provers) be used \ldots
  ...
  \item \ldots to automate (sub-)arguments? 
  \item \ldots to generate new knowledge?
  \end{itemize}
...
\begin{frame}{Inconsistency (KB): Reconstruction of Informal Argument}
...
\pause
\item[Def. D2$^*$] \hfill ${\ess{\phi}{x} \biimp {\allq \psi} (\psi(x)
    \imp {\nec} \allq y (\phi(y) \imp \psi(y)))}$ 
\pause
...
\begin{frame}{Inconsistency (KB): Verification in Isabelle/HOL}
\begin{changemargin}{-.5cm}{-1cm}
\includegraphics[width=12cm,height=8.5cm]{./DemoMaterial/GoedelGodWithoutConjunctInEss_KB.png}
\end{changemargin}
\end{frame}

\begin{frame}{Inconsistency (K): Verification in Isabelle/HOL}
\begin{changemargin}{-.5cm}{-1cm}
