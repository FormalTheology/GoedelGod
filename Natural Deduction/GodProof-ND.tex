...
$$
\item \textbf{(D2)} A property is \emph{essential} for an individual if and only if it holds for that individual and necessarily entails every other property that holds for that individual: $\ess{\varphi}{x} \biimp \varphi(x) \wedge \all \psi. (\psi(x) \imp \nec \all x. (\varphi(x) \imp \psi(x)))$
\end{itemize}
