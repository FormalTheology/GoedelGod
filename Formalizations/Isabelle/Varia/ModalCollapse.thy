...
section {* Modal Collapse *}  
  ...
  abbreviation collapse :: "\<sigma> \<Rightarrow> \<sigma>" ("collapse") where "collapse \<Phi> \<equiv> \<Phi> m\<rightarrow> (\<box> \<Phi>)"
  abbreviation collapse_dia :: "\<sigma> \<Rightarrow> \<sigma>" ("collapseDia") where "collapseDia \<Phi> \<equiv> (\<diamond> \<Phi>) m\<rightarrow> (\<box> \<Phi>)" 

  abbreviation L1 :: "\<sigma>" ("L1") where "L1 \<equiv> (collapse (\<exists>(\<lambda>x.(G x))))"
  abbreviation L2 :: "\<sigma>" ("L2") where "L2 \<equiv> (collapseDia (\<exists>(\<lambda>x.(G x))))"  


  theorem MC1: "\<forall>\<Phi>.\<forall>\<Psi>. [collapse \<Phi>] \<longrightarrow> [\<Psi> m\<rightarrow> \<Phi>] \<longrightarrow> [collapse \<Psi>]"
  nitpick [user_axioms]
  oops

  theorem MC2: "\<forall>\<Phi>.\<forall>\<Psi>. [collapse \<Phi>] \<longrightarrow> [\<Phi> m\<rightarrow> \<Psi>] \<longrightarrow> [collapse \<Psi>]"
  nitpick [user_axioms]
  oops

  theorem MC3: "\<forall>\<Phi>.\<forall>\<Psi>. [collapse \<Phi>] \<longrightarrow> [\<box> \<Phi> m\<rightarrow> \<box> \<Psi>] \<longrightarrow> [collapse \<Psi>]"
  nitpick [user_axioms]
  oops

  theorem MC4: "\<forall>\<Phi>.\<forall>\<Psi>. [collapse \<Phi>] \<longrightarrow> [\<Phi> m\<equiv> \<Psi>] \<longrightarrow> [collapse \<Psi>]"
  by metis

  theorem MC5: "\<forall>\<Phi>.\<forall>\<Psi>. [collapseDia \<Phi>] \<longrightarrow> [\<Phi> m\<rightarrow> \<Psi>] \<longrightarrow> [collapseDia \<Psi>]"
  nitpick [user_axioms]
  oops

  theorem MC6: "\<forall>\<Phi>.\<forall>\<Psi>. [collapse \<Phi>] \<longrightarrow> [(\<Phi> m\<rightarrow> \<Psi>) m\<rightarrow> (collapseDia \<Psi>)]"
  nitpick [user_axioms]
  oops

  theorem MC7: "\<forall>\<Phi>.\<forall>\<Psi>. [collapse \<Phi>] \<longrightarrow> [collapseDia (\<Phi> m\<or> \<Psi>)]"
  nitpick [user_axioms]
  oops
