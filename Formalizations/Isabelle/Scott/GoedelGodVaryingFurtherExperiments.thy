(*<*) 
theory GoedelGodVaryingFurtherExperiments
imports Main 
...
text {* The following lemmata show that existence is a positive property. Test2b and
Test3 do also hold in constant domain semantics. *}

  lemma Test1: "[P (\<lambda>x. (\<lambda>w. eiw w x))]"
  by (metis A1b A2)

  lemma Test2a: "[P (\<lambda>x.(\<exists>i(\<lambda>y. (\<lambda>w. x = y))))]"
  by (metis A1b A2)

  lemma Test2b: "[P (\<lambda>x.(\<exists>(\<lambda>y. (\<lambda>w. x = y))))]"
  by (metis A1b A2)

  lemma Test3: "[P (\<lambda>x.\<lambda>w. (\<exists>y. x = y))]"
  by (metis A1b A2)

text {* We continue with the main proof. *}
