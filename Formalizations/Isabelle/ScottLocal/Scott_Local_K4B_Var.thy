theory Scott_Local_K4B_Var imports QML_Local_Var
begin
...
(* A2: A property necessarily implied by a positive property is positive *)
   A2: "\<lfloor>\<^bold>\<forall>\<Phi> \<Psi>. P(\<Phi>) \<^bold>\<and> \<^bold>\<box>(\<^bold>\<forall>e x. \<Phi>(x) \<^bold>\<rightarrow> \<Psi>(x)) \<^bold>\<rightarrow> P(\<Psi>)\<rfloor>\<^sup>c\<^sup>w"

(* T1: Positive properties are possibly exemplified *)
 theorem T1: "\<lfloor>\<^bold>\<forall>\<Phi>. P(\<Phi>) \<^bold>\<rightarrow> \<^bold>\<diamond>(\<^bold>\<exists>e x. \<Phi>(x))\<rfloor>\<^sup>c\<^sup>w" using A1a A2 by blast
...
(* C: Possibly, God exists *) 
 corollary C: "\<lfloor>\<^bold>\<diamond>(\<^bold>\<exists>e x. G(x))\<rfloor>\<^sup>c\<^sup>w" by (metis A3 T1)
...
(* Ess: An essence of an individual is a property possessed by 
 ...
 definition ess (infixr "ess" 85) where
    "\<Phi> ess x = \<Phi>(x) \<^bold>\<and> (\<^bold>\<forall>\<Psi>. \<Psi>(x) \<^bold>\<rightarrow> \<^bold>\<box>(\<^bold>\<forall>e y. \<Phi>(y) \<^bold>\<rightarrow> \<Psi>(y)))"

(* T2: Being God-like is an essence of any God-like being *)
 theorem T2: "\<lfloor>\<^bold>\<forall>e x. G(x) \<^bold>\<rightarrow> G ess x\<rfloor>\<^sup>c\<^sup>w" by (smt A1b A4 G_def ess_def)

(* NE: Necessary existence of an individual is the necessary 
       exemplification of all its￼essences *)
 definition NE where "NE(x) = (\<^bold>\<forall>\<Phi>. \<Phi> ess x \<^bold>\<rightarrow> \<^bold>\<box>(\<^bold>\<exists>e x. \<Phi>(x)))"
...
(* T3: Necessarily, God exists *)
 theorem T3: "\<lfloor>\<^bold>\<box>(\<^bold>\<exists>e x. G(x))\<rfloor>\<^sup>c\<^sup>w" 
  proof -
   have L1: "\<lfloor>\<^bold>\<box>((\<^bold>\<exists>e x. G(x)) \<^bold>\<rightarrow> (\<^bold>\<box>(\<^bold>\<exists>e x. G(x))))\<rfloor>\<^sup>c\<^sup>w" 
proof -
  { fix ii :: i and mm :: \<mu> and iia :: i
    ...
    { assume "cw r ii"
      have "(\<exists>p. ((\<^sup>\<not>p) mm \<^bold>\<and> P p) ii) \<or> (\<^bold>\<not>op r cw) ii \<or> (\<^sup>\<not>G) mm ii \<or> (\<^bold>\<not>op r ii \<^bold>\<or> mexiB G) iia"
        using ff2 ff1 by (metis A1b A4 A5 NE_def trans)
...
qed
   ...
   hence "\<lfloor>\<^bold>\<diamond>(\<^bold>\<exists>e x. G(x))\<rfloor>\<^sup>c\<^sup>w \<Longrightarrow> \<lfloor>\<^bold>\<diamond>(\<^bold>\<box>(\<^bold>\<exists>e x. G(x)))\<rfloor>\<^sup>c\<^sup>w" by blast
   hence "\<lfloor>\<^bold>\<diamond>(\<^bold>\<exists>e x. G(x))\<rfloor>\<^sup>c\<^sup>w \<Longrightarrow> \<lfloor>\<^bold>\<box>(\<^bold>\<exists>e x. G(x))\<rfloor>\<^sup>c\<^sup>w" by (metis C sym trans)
   thus "\<lfloor>\<^bold>\<box>(\<^bold>\<exists>e x. G(x))\<rfloor>\<^sup>c\<^sup>w" using C by blast
  qed
