...
section {* Anderson's Ontological Argument -- varying domain only in the definition of NE *}   
  ...
  sledgehammer min [remote_satallax] (A1 A2 A3 Anderson_var_partial.refl Anderson_var_partial.sym Anderson_var_partial.trans C1 G_def NE_def T1 T2_lem ess_def nonempty)
  (* by (metis A1 A2 A3 Anderson_var_partial.refl Anderson_var_partial.sym Anderson_var_partial.trans C1 G_def NE_def T1 T2_lem ess_def nonempty) *)
  oops
...
section {* Provability of A4 and A5 *}
  ...
  by (metis A3 C2 G_def trans)
 

  theorem A5:  "[P NE]"
  nitpick [user_axioms = true]
  (* A5 is not provable anymore *)
  oops
