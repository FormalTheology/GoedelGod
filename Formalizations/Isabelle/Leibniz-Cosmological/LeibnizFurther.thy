...
lemma Cons:
assumes inf: "\<And>y x. hasInfiniteParts y \<Longrightarrow> x moves y \<Longrightarrow> movesTheInfinite x"  
and  Postulate_a: "\<And>P. (\<exists>x. P x) \<Longrightarrow> (\<exists>w. (\<forall>x.(P x \<longleftrightarrow> (x isPartOf w))))"
and  Axiom1: " \<And>x. (isMoved(x)) \<longrightarrow> ( \<exists>y. (y moves x))"  
and Axiom2: "\<forall>x. (moving x \<and> body x) \<longrightarrow> isMoved x"
and Axiom3: "\<And>y. (\<forall>x. (x isPartOf y \<longrightarrow> isMoved x)) \<longrightarrow> isMoved y" 
and  Axiom4: "\<forall>x. ((body x) \<longrightarrow> (hasInfiniteParts x))" 
