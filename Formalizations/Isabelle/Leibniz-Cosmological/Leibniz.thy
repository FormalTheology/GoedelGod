...
abbreviation isMover :: "\<mu> \<Rightarrow> bool"  where "(isMover x) \<equiv> (\<exists>y. (x moves y))"  
consts isMoved :: "\<mu> \<Rightarrow> bool"
...
consts hasInfiniteParts :: "\<mu> \<Rightarrow> bool" 
consts movesTheInfinite :: "\<mu> \<Rightarrow> bool"  

abbreviation infinitePower :: "\<mu> \<Rightarrow> bool" where 
"infinitePower x \<equiv>  movesTheInfinite x"
  
abbreviation substance :: "\<mu> \<Rightarrow> bool" where "substance x \<equiv> (\<exists>y. x moves y ) \<or> isMoved x"
...
axiomatization where inf: "hasInfiniteParts y \<Longrightarrow> x moves y \<Longrightarrow> movesTheInfinite x"  
...
one name is thought of which takes the place of all its parts in our reasoning[...]. This is called the Whole.\<acute>
...
axiomatization where Axiom2: "\<forall>x. (moving x \<and> body x) \<longrightarrow> isMoved x"
axiomatization where Axiom3: "(\<forall>x. (x isPartOf y \<longrightarrow> isMoved x)) \<longrightarrow> isMoved y" 
axiomatization where Axiom4: "\<forall>x. ((body x) \<longrightarrow> (hasInfiniteParts x))" 
...
then obtain mover where obtmover: "mover moves A" by auto
  ...
  {assume bm: "body mover"
    hence "isMoved mover" using moverMoves Axiom2 \<open>mover moves A\<close> by auto
(*Note that this step needs something like Add1 even in natural language.*)
{assume "\<not> (\<exists>x. (God x))"
  have "\<exists>C. (\<forall>x. (isMoved x \<and> body x ) \<longleftrightarrow> x isPartOf C)"
  proof -
    from obtmover bm have "\<exists>x. (\<lambda>l.(body l \<and> isMoved l)) x" using \<open>isMoved mover\<close> by auto
    hence "\<exists>C. (\<forall>x. (\<lambda>l.(body l \<and> isMoved l)) x \<longleftrightarrow> x isPartOf C)" using Postulate_a by simp
    then obtain C where obtC1: "(\<forall>x. (\<lambda>l.(body l \<and> isMoved l)) x \<longleftrightarrow> x isPartOf C)" by auto
    hence "(\<forall>x. (isMoved x \<and> body x ) \<longleftrightarrow> x isPartOf C)" by auto
    thus ?thesis by auto qed    
   then obtain C where obtC: " (\<forall>x. (isMoved x \<and> body x ) \<longleftrightarrow> x isPartOf C)" by blast
   have "isMoved C" using Axiom3 using \<open>\<forall>x. (isMoved x \<and> body x) = x isPartOf C\<close> by blast
   hence "\<exists>G. G moves C" using Axiom1 by simp
   from this obtain G where "G moves C" by auto
   have "incorporeal G"  using moverMoves  Axiom2 \<open>G moves C\<close> \<open>\<forall>x. (isMoved x \<and> body x) = x isPartOf C\<close>  NoPartMoves by blast
   have "substance G" using \<open>G moves C\<close> using moverMoves NoPartMoves Axiom2 \<open>\<forall>x. (isMoved x \<and> body x) = x isPartOf C\<close> by blast
