...
\imp P(\psi)]$ (A property necessarily implied by a positive property is positive). *}
  ...
  axiomatization where
    A1a: " \<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>. P v w (\<lambda>m. \<lambda>n. (\<lambda>x. m\<not> (\<Phi> m n x))) m\<rightarrow> m\<not> (P v w \<Phi>))) v w)"
  axiomatization where   
    A1b: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>. m\<not> (P v w \<Phi>) m\<rightarrow> P v w (\<lambda>m. \<lambda>n. (\<lambda>x. m\<not> (\<Phi> m n x))))) v w)" 
  axiomatization where    
    A2:  "\<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>. \<forall>(\<lambda>\<Psi>. (P v w \<Phi> m\<and> \<box> (\<forall>(\<lambda>x. \<Phi> v w x m\<rightarrow> \<Psi> v w x))) m\<rightarrow> P v w \<Psi>))) v w)"
...
that is verified in Isabelle/HOL's kernel. *}
  ...
  theorem T1: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>. P v w \<Phi> m\<rightarrow> \<diamond> (\<exists> (\<Phi> v w)))) v w)"  
...
(Possibly, God exists). *} 
  ...
  axiomatization where A3:  "\<forall>v. \<forall>w. (((P v w) G) v w)" 

  corollary C: "\<forall>v. \<forall>w. ((\<exists> (G v w)) v w)" 
  -- {* sledgehammer [provers = remote\_leo2] *}
  oops
...
(Positive properties are necessarily positive). *}
  ...
  axiomatization where A4:  "\<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>. P v w \<Phi> m\<rightarrow> \<box> (P v w \<Phi>))) v w)" 
...
\imp \psi(y)))$$ (An \emph{essence} of an individual is a property possessed by it and necessarily implying any of its properties). *}
  ...
  definition ess :: "i \<Rightarrow> i \<Rightarrow> (\<mu> \<Rightarrow> \<sigma>) \<Rightarrow> (\<mu> \<Rightarrow> \<sigma>)" where
    "ess = (\<lambda>v. \<lambda>w. \<lambda>\<Phi>. \<lambda>x. (\<Phi> x) m\<and> \<forall>(\<lambda>\<Psi>. \<Psi> x m\<rightarrow> \<box> (\<forall>(\<lambda>y. \<Phi> y m\<rightarrow> \<Psi> y))))"
...
(Being God-like is an essence of any God-like being). *}
  ...
  theorem T2: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>x. G v w x m\<rightarrow> ess v w (G v w) x)) v w)"
  -- {* sledgehammer [provers = remote\_leo2] *}
  -- {* metis is too weak: by (metis A1b A4 G_def ess_def) *}
  oops
  
  axiomatization where T2: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>x. G v w x m\<rightarrow> ess v w (G v w) x)) v w)"
...
existence of an individual is the necessary exemplification of all its essences). *}
  ...
  definition NE :: "i \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> \<sigma>" where "NE = (\<lambda>v. \<lambda>w. \<lambda>x. \<forall>(\<lambda>\<Phi>. ess v w (\<Phi> v w) x m\<rightarrow> \<box> (\<exists> (\<Phi> v w))))"

text {* Moreover, axiom @{text "A5"} is added: $P(\NE)$ (Necessary existence is a positive 
property). *}

  axiomatization where A5:  "\<forall>v. \<forall>w. ((P v w NE) v w)"
...
(Necessarily, God exists). *}
  ...
  theorem T3: "\<forall>v. \<forall>w. ((\<box> (\<exists> (G v w))) v w)"
  unfolding G_def
  -- {* sledgehammer [provers = remote\_leo2] *}
  oops
...
(reflexivity). *}
  ...
  corollary C2: "\<forall>v. \<forall>w. ((\<exists> (G v w)) v w)" 
  -- {* sledgehammer [provers = remote\_leo2] *}
  oops
...
text {* G\"odel's God is flawless: (s)he does not have non-positive properties. *}
  ...
  theorem Flawlessness: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>. \<forall>(\<lambda>x. (G v w x m\<rightarrow> (m\<not> (P v w \<Phi>) m\<rightarrow> m\<not> (\<Phi> v w x)))))) v w)"
  -- {* sledgehammer [provers = remote\_leo2] *}
  by (metis A1b G_def) 
  
text {* There is only one God: any two God-like beings are equal. *}   
  
  theorem Monotheism: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>x.\<forall>(\<lambda>y. (G v w x m\<rightarrow> (G v w y m\<rightarrow> (x mL= y)))))) v w)"
  -- {* sledgehammer [provers = remote\_leo2] *}
  oops
...
Calling Metis with @{text "T2"}, @{text "T3"} and @{text "ess_def"} also does not work. *} 
  ...
  lemma MC: "\<forall>v. \<forall>w. ((\<forall>(\<lambda>\<Phi>.(\<Phi> m\<rightarrow> (\<box> \<Phi>)))) v w)"  
