(*<*) 
theory GoedelGodIntensionalVarying
imports Main 
...
and @{text "\<diamond>"}, while the second world is dynamically modified. *}
  ...
  abbreviation  mforall_ind :: "(\<mu> \<Rightarrow> \<sigma>) \<Rightarrow> \<sigma>" ("\<forall>i") where "\<forall>i P \<equiv> (\<lambda>v. \<lambda>w. \<forall>x. eiw w x \<longrightarrow> P x v w)"
  abbreviation  mexists_ind :: "(\<mu> \<Rightarrow> \<sigma>) \<Rightarrow> \<sigma>" ("\<exists>i") where "\<exists>i P \<equiv> (\<lambda>v. \<lambda>w. \<exists>x. eiw w x \<and> P x v w)"
...
(reflexivity). *}
  ...
  -- {* sledgehammer [provers = remote\_leo2] *}
  by (metis (lifting) A1b GoedelGodIntensionalVarying.sym T1 T3)
