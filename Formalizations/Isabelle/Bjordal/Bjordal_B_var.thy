...
D is implied. *}
 ...
 lemma Case1: "\<lfloor>\<^bold>\<forall>(\<lambda>\<Phi>. (\<^bold>\<box>(\<^bold>\<forall>\<^sup>E(\<lambda>x. G x \<^bold>\<rightarrow> \<Phi> x))) \<^bold>\<rightarrow> P \<Phi>)\<rfloor>"
 (* sledgehammer [provers = remote_leo2 remote_satallax] *)
 by (metis A2 A3) 

 lemma Case2: "\<lfloor>\<^bold>\<forall>(\<lambda>\<Phi>. P \<Phi> \<^bold>\<rightarrow> (\<^bold>\<box>(\<^bold>\<forall>\<^sup>E(\<lambda>x. G x \<^bold>\<rightarrow> \<Phi> x))))\<rfloor>"
 (* sledgehammer [provers = remote_leo2 remote_satallax] *)
 by (metis A4 G_def) 

 theorem D1: "P = (\<lambda>\<Phi>. \<^bold>\<box>(\<^bold>\<forall>\<^sup>E(\<lambda>x. G x \<^bold>\<rightarrow> \<Phi> x)))"
 (* sledgehammer [provers = remote_leo2 remote_satallax] *)
