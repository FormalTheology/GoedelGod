...
lemma IDEN1: "A \<^bold>= A" (* This is not needed as an axiom. *)
by(simp add: CONT1 equal_def)

lemma equal_sym: "A \<^bold>= B \<equiv> B \<^bold>= A"
using equal_def
by smt
...
lemma NEG4: "A \<^bold>= B \<Longrightarrow> \<not> (A \<^bold>= \<^bold>~ B)"
using CONT2 NEG3 equal_def
by blast

lemma NEG5: "P A \<Longrightarrow> (A \<^bold>\<notin> \<^bold>~ A)"
by(simp add: CONJ1 CONT1 possible_def notcontains_def)

lemma NEG6: "P A \<Longrightarrow> A \<^bold>\<in> B \<Longrightarrow> \<not> (A \<^bold>\<in> \<^bold>~ B)"
...
by(meson CONJ4 CONJ5 NEG6 POSS1 POSS2)
