...
(* Jan 2014: Modified by Chad E. Brown (BM-2DA9t7SbiWNhctUpCGaozqojK4aNWwH4mm) as follows:
   The axiom axiom3 and its consequences are commented out, then
   axiom3 is proven to follow from corollary2 (existence of God)
   when corollary2 is assumed as an axiom.
   In fact, only axiom1(b) is used to prove that axiom3 follows from existence of God.
   Assuming all the axioms except axiom3, we could prove
   axiom3 is equivalent to corollary2.
   That is, being God-like is Positive if and only if a God exists.
   Or, in other words, assuming being God-like is Positive is
   equivalent to assuming God exists.
 *)
...
(***
   Axiom A3 and Corollary C1 commented out by Chad.
   Axiom A3 is proven assuming existence of God below. ***)
(***
...
(*** Commented out by Chad, since it depends on corollary1 which was proven using axiom3.
...
(* Chad: Changed this to be an axiom in order to prove Axiom A3 from it. ***)
Axiom corollary2: V (mexists x, (G x)).
(***
Proof.
intro.
apply (theorem3 w).
...
Lemma axiom3 : V (Positive G).
Proof.
  intros w. destruct (corollary2 w) as [x H1].
  apply NNPP. intros H2.
  generalize (axiom1b w G H2).
  intros H3.
  generalize (H1 (fun x u => ~G x u) H3).
  tauto.
Qed.