...
ontological argument for the existence of God. More precisely, we
further expand our studies on modern (modal) variants of the
ontological argument that are essentially emendations of Kurt Gödel's
seminal contribution \citep{GoedelNotes}, respectively, Dana Scott's
variant of it \cite{ScottNotes}. In related experiments
\citep{C55,C40,J30}, we have already demonstrated that our technology
is suited not only to formally assess the logical quality of the
presented arguments, but it may even generate new knowledge about
it. For example, the theorem prover Leo-II \cite{Leo-II} detected an
inconsistency in Gödel's original variant \citep{GoedelNotes} of the
argument, which is not the case for Scott \citep{ScottNotes}.  
We here assume some familiarity with these related experiments. 

The particular motivation and context for work presented in this
article is as follows: The axioms in G\"odel's ontological proof
\citep{GoedelNotes,ScottNotes} (cf. Fig. \ref{fig:Goedel}) entail what
is called \emph{modal collapse} \citep{Sobel1987,SobelBook2004}: the
formula $\varphi \rightarrow \Box \varphi$, abbreviated as MC, holds
for any formula $\varphi$ and not just for
$\exists x. \mathit{God}(x)$ as intended.
\begin{figure}[t]
...
\end{figure}
This fact, which has been confirmed with higher-order automated
theorem provers \citep{C40,J30} in our previous experiments, has led
to strong criticism of the argument and stimulated attempts to remedy
the problem.
...
formalized those using the proof assistant Isabelle/HOL
\citep{Isabelle} together with the automated higher-order reasoners
Leo-II \citep{Leo-II}, Satallax \citep{brown2012satallax}, Metis
\citep{Hurd03first-orderproof}, and Nitpick \citep{Nitpick}.  Our
formalizations employ the embedding of higher-order modal logic (HOML)
in classical higher-order logic (HOL) as introduced in previous work
\citep{C40,J23}. We explored the effect of different domain conditions
on the provability of lemmas, theorems and even axioms.  This was
motivated by a controversy between Hájek and Anderson regarding the
redundancy of some axioms in Anderson's emendation. In \emph{constant
  domain semantics}, the individual domains are the same in all
possible worlds. In \emph{varying domain semantics}, the domains may
vary from world to world. This variation is technically encoded with
the help of an existence relation expressing which individuals
actually exist in each world. Quantifiers are then uniformly
formalized as \emph{actualistic quantifiers} (i.e. guarded by the
existence relation). \footnote{EDNOTE: This is misleading statement;
  we need to reword}
Our main results, which we have presented at the
1st World Congress on Logic and Religion \citep{C41}, are summarized
below. The corresponding formalizations, containing all the details,
are available in the subdirectories \url{Anderson}, \url{Hajek} and
\url{Bjordal} at
\url{github.com/FormalTheology/GoedelGod/blob/master/Formalizations/Isabelle/}.



\section{Results from our Experiments}
...
\begin{figure}
\begin{sideways}
\includegraphics[width=1.2\textwidth]{Summary.png} (to be removed)
\end{sideways}
%
\begin{sideways}
\scalebox{.9}{
...
\section{Conclusion}
Using our approach (semantical embeddings of variants of higher-order
...
provers not only confirmed many claimed results, but also exposed a
...
adopted in larger scale by philosophers. Moreover, the technology is
by now means restricted to metaphysics or philosophy, and further work
