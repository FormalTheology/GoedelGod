...
\author{Annika Kanckos \and Bruno Woltzenlogel Paleo\thanks{Author order is alphabetical by surname.}}

\authorrunning{A.\~Kanckos \and B.\~Woltzenlogel Paleo}
...
point for two distinct but complementary research endeavours. Kanckos
...
Bj\o rdal and H\'ajek were presented in the First
...
\begin{calculus}
{Admissible propositional rules}
{fig:AdmissiblePropositionalRules}
...
\infer[\neg_E]{\bot}{ \neg A & A }
\s\s
\infer[\biimp_E]{A}{A\biimp B & B  }
\s\s
\infer[\biimp_E]{B}{A\biimp B & A  }
...
{\NDK} is complete relative to {\ND} extended with Axiom K and necessitation.
\end{theorem}
\begin{proof}
...
{\NDK} is sound relative to {\ND} extended with Axiom K and necessitation.
\end{theorem}
\begin{proof}
...
The \emph{reflexivity} of equality is a theorem in higher-order logics where equality is defined as Leibniz's equality. A natural deduction proof can be easily constructed, as shown below.
...
\begin{prooftree}
\AXC{}\RightLabel{$1$}
\UIC{$P(\alpha)$}\RightLabel{$\imp_I^1$}
\UIC{$P(\alpha) \imp P(\alpha)$}
\AXC{}\RightLabel{$2$}
\UIC{$P(\alpha)$}\RightLabel{$\imp_I^2$}
\UIC{$P(\alpha) \imp P(\alpha)$}\RightLabel{$\wedge_I$}
\BIC{$(P(\alpha) \imp P(\alpha)) \wedge (P(\alpha) \imp P(\alpha))$} \dottedLine
\UIC{$P(\alpha) \biimp P(\alpha)$}\RightLabel{$\forall_I$}
\UIC{$\forall P. P(\alpha) \biimp P(\alpha)$}\dottedLine
\UIC{$\alpha=\alpha$} \RightLabel{$\forall_I$}
\UIC{$\forall x. x=x$} 
\end{prooftree}
\end{footnotesize}
...
\UIC{$P(\rho)$}
    ...
    \UIC{$\omega: \forall x. x = x$} \RightLabel{$\forall_E$}
	...
	\BIC{$P(\rho) \wedge \nec \all x. (\rho(x) \imp x = x) $} 
	    	...
	    	\UIC{$P(\rho) \wedge \nec \all x. (\rho(x) \imp x = x) \imp P(\lambda x. x = x)$} \RightLabel{$\imp_E$}
		...
		\UIC{$P(\lambda x. x \neq x)\scott$}    
    		\AXC{$\subproof_1$} \dashedLine
			  \UIC{$P(\lambda x. x = x)\scott$} 
					\AXC{Contraposition of Half of Axiom 1} \dashedLine
...
\UIC{$\omega: \neg \ex x. \rho(x)$}
    ...
    \BIC{$\omega: \bot$} \RightLabel{$\imp_I^2$}
	\UIC{$\omega: \neg \rho(\alpha)$} \RightLabel{$\all_I$}
	\UIC{$\omega: \all x. \neg \rho(x)$} \RightLabel{$\nec_I$}
	\UIC{$\nec \all x. \neg \rho(x) \scott$}
	        \AXC{$\subproof_3 $} \dashedLine
	        \UIC{$\bot$} \RightLabel{$\imp_I^3$}
    		\UIC{$\neg \nec \all x. \neg \rho(x)$} \RightLabel{$\neg_E$}
  		\BIC{$\bot$} \RightLabel{$\imp_I^4$}
...
\begin{prooftree}
		...
		\UIC{$\nec (\ex x. G(x) \imp \nec \ex x. G(x)) $} 
			...
			\BIC{$[\subproof_5] \qquad \pos \ex x. G(x) \imp \pos \nec \ex x. G(x)$} \RightLabel{$\imp_E$}
...
\UIC{$\pos \ex x. G(x)$} 
 \AXC{$\subproof_5$}\dashedLine
 	\UIC{$\pos \ex x. G(x) \imp \pos \nec \ex x. G(x)$} \RightLabel{$\imp_E$}
 \BIC{$\pos \nec \ex x. G(x)\scott$} 
			\AXC{{\bf S5} Iteration Principle} \dashedLine
			\UIC{$\pos \nec \ex x. G(x) \imp \nec \ex x. G(x)$}\RightLabel{$\imp_E$}
		\BIC{$\nec \ex x. G(x)$} \RightLabel{$\imp_I^1$}
...
\BIC{$[\subproof_6]\qquad P(\lambda x. \neg \rho(x))$} \RightLabel{$\imp_E$}
\end{prooftree}
\end{scriptsize}
...
\UIC{$\ex x. G(x)\imp \nec \ex x. G(x)$}\RightLabel{$\nec_I$}
\UIC{$\nec(\ex x. G(x)\imp \nec \ex x. G(x))$}\RightLabel{$\imp_E$}
\BIC{$[\subproof_9] \qquad \pos \ex x. G(x)\imp \pos\nec \ex x. G(x)$}
\end{prooftree}
\end{small}
\begin{small}
\begin{prooftree}
...
Anderson \citep[footnote 5]{and90} cites several articles questioning the adequacy of {\bf S5} for the ontological argument and conjectures that the ontological argument could be restricted to the weaker modal logic {\bf B}. Sobel \citep[p. 152]{sobel2} acknowledges Anderson's conjecture, claims that it is correct and tries to informally explain how his formal proofs could be modified to rely on {\bf B} instead of {\bf S5}. Nevertheless, neither Anderson nor Sobel presented any formal proof relying only on {\bf B}. The new formal proof presented in this section relies on {\bf KB}, which is weaker than {\bf B}. 

Curiously, in the new proof of Theorem 3, the actual existence of a God-like being ($\ex x. G(x)$) is proven \emph{before} its necessity ($\nec \ex x. G(x)$). This contrasts with G\"odel's proof, where the actual existence is never actually proven (although it could, nevertheless, be derived as a trivial corollary by applying the modal Axiom T).
...
Below we show natural deduction derivations of the modal collapse, thereby confirming that it holds\footnote{It is well-known and uncontroversial that the modal collapse holds. We show a natural deduction proof here merely for the sake of self-containment and so that the proof becomes more accessible to readers who might be more familiarized with natural deduction than with Sobel's proof system.} for the axioms used in the previous sections. We show only the strongest implication above. The others are easy corollaries in sufficiently strong modal logics (e.g. where, for instance, $\nec A \imp \pos A$ or $\pos \top$ holds).
...
\UIC{$[\subproof_{10}] \qquad \ex y.G(y) \imp (A \imp \nec \all x. (G(x) \imp A))$}
\end{prooftree}
\end{small}
...
Because of Anderson's emendations to Axiom 1 and Definitions 1 and 2, the proof of Lemma 1 has to be modified\footnote{Due to lack of space, the instantiation of Axiom \ref{A:A2} is omitted in the emended proof of Lemma \ref{A:L1}.} as well.
The proof shown below has been modified as little as possible. Interestingly, in order to eliminate 
the extra box modality in Anderson's emended Definition \ref{A:D1}, the modified proof below uses the modal axiom T. 
Likewise, to obtain the extra box in the emended Definition \ref{A:D2}, Axioms 2 and 3 were used.
...
\AXC{$ $} \RightLabel{1}
\UIC{$\ex z. G(z)$}\RightLabel{$\ex_E$}
\UIC{$[\subproof_{11}] \quad G(\gamma)$}
\end{prooftree}

\begin{prooftree}
\AXC{$\subproof_{11}$} \dashedLine
...
\UIC{$\omega: P(\rho) \imp \nec \rho(y)$}\RightLabel{$\imp_E$}
\BIC{$\omega: \nec \rho(y)$}  
    \AXC{$\omega:$ Axiom T} \dashedLine
    \UIC{$\omega: \nec \rho(y) \imp \rho(y)$} \RightLabel{$\imp_E$}
\BIC{$\omega: \rho(y)$} \RightLabel{$\imp_I^2$}
\UIC{$\omega: G(y) \imp \rho(y)$}\RightLabel{$\forall_I$}
\UIC{$\omega: \forall y.(G(y) \imp \rho(y))$}\RightLabel{$\nec_I$}
\UIC{$\nec \forall y.(G(y) \imp \rho(y))$} \RightLabel{$\imp_I^3$}
\UIC{$\nec P(\rho)\imp \nec \forall y .(G(y)\imp \rho(y))$} 

\AXC{$\subproof_{12}$} \dashedLine
\UIC{$\nec P(\rho)$}\RightLabel{$\imp_E$}
\BIC{$\nec \forall y .(G(y)\imp \rho(y))$}\RightLabel{$\imp_I^4$}
\UIC{$[\subproof_{13}] \qquad \nec\rho(\gamma) \imp \nec \forall y.(G(y) \imp \rho(y))$}\RightLabel{$\forall_I$}
\end{prooftree}
\end{small}
...
\AXC{Axiom \ref{A:A2}} \dashedLine
\UIC{.} \doubleLine \RightLabel{$\all_E$ x 2}
\UIC{.}\RightLabel{$\imp_E$}
\BIC{$P(\varphi)$}
\AXC{$\subproof_{11}$} \dashedLine 
\UIC{$  G(\gamma)$} \dottedLine \RightLabel{D\ref{A:D1}}
...
the proof of Lemma \ref{A:L1}, which is the only part of the argument that depended on Axioms \ref{A:A4} and \ref{A:A5} and Definitions \ref{A:D2} and \ref{A:D3}. The modified proof is shown below. In contrast to the previous proof, the shorter proof does not use the modal axiom T. Therefore, modal logic \textbf{KB} is sufficient for Anderson's emendation as well.
...
\UIC{$\forall \varphi. (P(\lambda x.\varphi(x)) \biimp \nec \varphi(\gamma))$}\RightLabel{$\forall_E$}
\UIC{$P(G) \biimp \nec G(\gamma)$} \RightLabel{$\biimp_E$}
\AXC{Axiom \ref{A:A3}} \dashedLine \RightLabel{}
...
%\UIC{$\all y.(G(y) \imp (\nec A(y) \biimp \nec \all x. (G(x) \imp A(x))))$} \RightLabel{$A$ is constant}
\UIC{$\all y.(G(y) \imp (\nec A \biimp \nec \all x. (G(x) \imp A)))$} \doubleLine
\UIC{$[\subproof_{15}] \qquad \ex y.G(y) \imp (\nec A \biimp \nec \all x. (G(x) \imp A))$}
\end{prooftree}
\end{small}
...
\UIC{$\omega: \ex x.G_B(x) $} \RightLabel{$\neg_E$}
\BIC{$\omega:  \bot $}\RightLabel{$\imp_I^1$}
...
\begin{prooftree}
\AXC{$ $} \RightLabel{$2$}
\UIC{$\ex x.G_B(x) $} \RightLabel{ $\ex_E$}
\UIC{$G_B(x) $} 
\AXC{$ $}\doubleLine
\UIC{$\nec \all x. (G_B(x) \imp G_B(x)) $}\RightLabel{ D\ref{B:D1}}\dottedLine
\UIC{$P_B(G_B) $}\RightLabel{ $\wedge_I$}
\BIC{$[\subproof_{16}] \qquad G_B(x) \wedge P_B(G_B) $}
\end{prooftree}
\end{small}

\begin{small}
\begin{prooftree}
\AXC{$ \subproof_{16} $}\dashedLine
...
\UIC{$[\subproof_{17}] \qquad \MCP(G_B, x) $} 
...
The formal proofs presented here are short and accessible because of the choice of a natural deduction calculus. The new proof of section \ref{sec:newproof} shows explicitly that the weaker modal logic {\bf KB} is sufficient for the ontological argument. The stronger modal logic {\bf S5}, which is usually taken for granted in discussions of G\"odel's ontological argument, is not necessary. This weakening of the underlying logic confirms and strengthens a claim briefly mentioned by Anderson \citep[footnote 2]{and90}. {\bf KB} is also sufficient for the variants of Anderson and Bj{\o}rdal, which address Sobel's criticism of the modal collapse.

\paragraph{Acknowledgements:} We thank Padre Edvaldo (from Piracicaba, Brazil) for assisting Bruno and his family in a moment of need. Gratitude for his assistance is what triggered the production of an initial draft of this article in early 2013. We are thankful to Christoph Benzm\"uller for collaborations on several related articles. This work was supported by a Stipendium of the \"Osterreichische Akademie der Wissenschaften (APART). 
...
Anderson, C. A.\& Gettings, M. 1996.  {\itshape G\"odel's ontological proof revisited}. In: edited by H\'ajek P. {\itshape G\"odel '96},  Springer. 

\bibitem[{\itshape Bentert \& Benzm\"uller \& Streit \& Woltzenlogel Paleo(2016)}]{LeibnizOntologicalProof}
...
\bibitem[{\itshape Benzm\"uller \& Weber \& Woltzenlogel Paleo(2015)}]{WorldCongressLogicAndReligion}
Benzm\"uller, C. \& Weber, L. \& Woltzenlogel Paleo, B. 2015. {\itshape Computer-Assisted Analysis of the Anderson-H\'ajek Ontological Controversy}. Handbook of the 1st World Congress on Logic and Religion. Eds. Silvestre, R. S. \& B\'{e}ziau, J-Y. 

\bibitem[{\itshape Benzm\"uller \& Weber \& Woltzenlogel Paleo(2017)}]{controversy}
...
%\bibitem[{\itshape Benzm\"uller \& Weber \& Woltzenlogel Paleo(2016)}]{AndersonHajekControversy}
%Benzm\"uller, C. \& Weber, L. \& Woltzenlogel Paleo, B. 2016. {\itshape Computer-Assisted Analysis of the Anderson-Hajek Ontological Controversy}. Logica Universalis. Springer. 

\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2013a)}]{IWIL}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2013b)}]{IsabelleGod}
Benzm\"uller, C. \& Woltzenlogel Paleo, B. 2013b. {\itshape G{\"{o}}del's God in Isabelle/HOL}. Archive of Formal Proofs. 

\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2014a)}]{WADT}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2014b)}]{WADTPreProceedings}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2014c)}]{SquareOfOpposition}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2014d)}]{ECAI2014}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2015a)}]{CSRModalLogicInCoq}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2015b)}]{ReasoningWebSummerSchool}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2015c)}]{SpiritualQuest}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2016a)}]{IJCAI2016}
...
\bibitem[{\itshape Benzm\"uller \& Woltzenlogel Paleo(2016b)}]{ModalCollapse}
...
\bibitem[{\itshape G\"odel(1970)}]{Goedel} 
...
\bibitem[{\itshape H\'ajek(1996)}]{hajek}
H\'ajek, P. 1996. Magari and others on G\"odel's Ontological Proof. Eds. Ursini et al. Logic and Logical Algebra, pp. 125--136, Marcel Dekker.
...
\bibitem[{\itshape Muskens(2006)}]{Muskens}
...
\bibitem[{\itshape Paulin-Mohring(2015)}]{coq} Paulin-Mohring, C. 2015. {\itshape Introduction to the calculus of inductive constructions}. Eds. Delahaye, D. \& Woltzenlogel Paleo, B. 
...
\bibitem[{\itshape Thiele(2003)}]{hilbert} Thiele, R. 2003. {\itshape Hilbert's twenty-fourth problem}, American Mathematical Monthly, January 2003. 

\bibitem[{\itshape Wang(1996)}]{Wang1996}
Wang, H. 1996. {\itshape A Logical Journey: From G\"odel to Philosophy}, The MIT Press. 

\bibitem[{\itshape Woltzenlogel Paleo(2013)}]{AustrianComputerSociety}
