...
This paper presents new natural deduction proofs of (Scott's version of) the lemmas and theorems in G\"odel's ontological argument \cite{Goedel}. In contrast to Scott's version \cite{Scott}, also shown here in natural deduction form, the version presented here is characterized by shorter and more direct derivations.

Section \ref{sec:calculus} defines the higher-order modal natural deduction calculus used to construct the formal proofs. Section \ref{sec:scott} shows Scott's proofs, translated to natural deduction. And section \ref{sec:proof} presents the new proofs.
...
\noindent
A \emph{derivation} is a directed acyclic graph whose nodes are formulas and whose edges correspond to applications of the inference rules shown in Figures \ref{fig:ND}, \ref{fig:Classical} and \ref{fig:NDK}. Parts of a derivation may be surrounded by boxes. A \emph{proof} is a derivation that additionally satisfies the following conditions:
...
%
\item \textbf{eigen-box conditions:} $\omega$ must be a fresh name for a box.
%
