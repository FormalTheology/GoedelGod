...
\begin{minipage}{.94\columnwidth}\small
\begin{itemize}
\item[H:DP] A \emph{God-like} being necessarily possesses those and only those properties that are necessarily implied by a positive property:
  $$\hol{P(x) \equiv \forall \varphi [\nec \varphi(x) \biimp \exq \psi[P(\psi) \wedge \nec \allq x [\psi(x)  \imp \varphi(x))]]}$$
\item[A3']  The property of being God-like is positive:
  $$\hol{P(G_H)}$$
\item[A4]  Positive properties are necessarily positive:
  $$\hol{\allq \varphi [P(\varphi) \imp \Box \; P(\varphi)]}$$
...
aswell as independent. For the old A5', no conclusive results were achieved.
