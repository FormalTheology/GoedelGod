...
In addition to the five axioms and three definitions that pertain specifically to the argument, the proof also uses {\bf S5}'s iteration principle (in the proof of Lemma 2) and the equality axiom of reflexivity (in the proof of Theorem 1). The use of reflexivity could be avoided, but we chose to use it because it allows the construction of a shorter proof.
...
\UIC{$P(E)$}\dottedLine \RightLabel{D\ref{N:D3}}
\AXC{$\subproof_6$} \dashedLine
\UIC{$G(\gamma)$}\dottedLine \RightLabel{D\ref{N:D1}}
\UIC{$\forall \varphi.(P(\varphi) \imp \varphi(\gamma))$}\RightLabel{$\forall_E$}
...
\end{theorem}
\begin{proof}
This proof is omitted, because any of the proofs of Theorem 1 in Sections \ref{sec:scott} and \ref{sec:newproof} would suffice to prove this theorem with only small modifications: the equivalence of axiom \ref{A1} would not need to be reduced to an implication by conjunction elimination. 
\end{proof}
...
ToDo: Proof of $\star_7$ is missing in the proof of lemma 1 below.
...
\AXC{$ $} \RightLabel{6}
\UIC{$G(y) $}\dottedLine \RightLabel{D\ref{A:D1}}
\UIC{$\forall \varphi. [P(\lambda x.\varphi(x)) \biimp \nec \varphi(y)]$}
\UIC{$[P(\lambda x.\rho(x)) \biimp \nec \rho(y)]$}
\BIC{$\nec \rho(y)$}\RightLabel{$\nec_E$}
...
\begin{small}
\begin{prooftree}
\AXC{$\subproof_7$ and $\subproof_8$} \dashedLine
...
\BIC{$ E(\gamma)$} \dottedLine \RightLabel{D\ref{A:D3}}
\UIC{$ \all \varphi.[\ess{\varphi}{\gamma} \imp \nec \ex x.\varphi(x)] $}\RightLabel{$\all_E$}
\UIC{$ \ess{G}{\gamma} \imp \nec \ex x. G(x) $}\RightLabel{$\imp_E$}
\BIC{$\nec \ex x. G(x)$} \RightLabel{$\imp_I^1$}
\UIC{$\ex z. G(z) \imp \nec \ex x. G(x)$}
...
We know from \citep{hajek} that Anderson's emendation also makes Axioms 4 and 5 derivable from the other axioms and definitions. Discussions about the redundancy of these two axioms started with an earlier (and wrong claim) claim by Magari \citep{magari} w.r.t. Scott's variant, and the redundancy was the topic of a long controversy \citep{controversy} between H\'ajek and Anderson. The proof presented above still uses Axioms 4 and 5, though, since the intention here was to show that Anderson's emendation requires only a minimal modification of the proofs presented in the previous sections.

\subsection{A shorter proof without redundant axioms}
...
There is an alternative proof of theorem \ref{A:T3} using only Anderson's axioms \ref{A:A1}, \ref{A:A2} and \ref{A:A3} as well as definition \ref{A:D1}. %Hájek1996 could be helpful.
The proof requires only a modification of the proof of lemma \ref{A:L1}. The rest of the dependencies are as in the previous proof. 

Theorem \ref{A:T1} is derived from axiom \ref{A:A1} and axiom \ref{A:A2}. Corollary \ref{A:C1} is derived from axiom \ref{A:A3} and theorem \ref{A:T1}. The proof of lemma \ref{A:L1} can be modified, so as to be derived from axiom \ref{A:A3} using definition \ref{A:D1}. Finally, theorem \ref{A:T3} can be derived from lemma \ref{A:L1} and corollary \ref{A:C1}. 
...
\UIC{$\ex x.G(x)$}  \RightLabel{$\imp_I^5$}
\UIC{$G(\gamma)\imp \ex x.G(x)$} \RightLabel{$\nec_I$}
\UIC{$\nec [G(\gamma)\imp \ex x.G(x)]$}  \RightLabel{Axiom K}
\UIC{$\nec G(\gamma)\imp \nec \ex x.G(x)$} \RightLabel{$\imp_E$}
\BIC{$\nec \ex x.G(x)$}\RightLabel{$\imp_I^1$}
\UIC{$\ex z. G(z)\imp \nec \ex x.G(x)$}
\end{prooftree}
...
Bj{\o}rdal refines the notion of essence (which he calls maximal composite) by restricting the attention to positive properties only. He calls this refined notion of essence a maximal composite.
...
ToDo: State corollary and prove it, instead of making a forward reference to lemma L1. Then use the corollary in lemma 1.

Corollary 1 of section \ref{sec:newproof} follows by proving the positivity of godlikeness as in lemma \ref{B:L1} below. 
